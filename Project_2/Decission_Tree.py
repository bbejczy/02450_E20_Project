...
def classifier_model(X,y,K,yhat,ytrue):
    # split data
    K = 10
    CV = KFold(K, shuffle=True)
    # X_train, X_test, y_train, y_test = KFold(X,y, test_size=0.33)
...
def regressor_model(X,y,K,yhat,ytrue):
    # split data 
    # X_train, X_test, y_train, y_test = train_test_split(X,y, test_size=0.33)
...
classifier_complexity(X, y)

K = 10
yhat = []
ytrue = []
Error_train,Error_train,yhat,ytrue=classifier_model(X, y, K, yhat, ytrue)
...
regressor_complexity(X,y)

regressor_model(X,y,K,yhat,ytrue)
